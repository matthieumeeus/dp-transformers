...
for multiple in "${multiples[@]}"; do
    for method in "${mia_methods[@]}"; do
        ...
        python estimate_privacy_synthetic.py --config-name synthetic_agnews_syntheticcanary_uniformlabel +submit=True ++shared_training_parameters.synthetic_multiple=${multiple} ++shared_inference_parameters.mia_method=${method}
    done
done
